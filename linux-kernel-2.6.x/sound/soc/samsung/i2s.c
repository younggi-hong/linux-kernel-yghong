	...
	struct snd_soc_dai *dai)
{
	...
	/* Gate CDCLK by default */
        if (!is_opened(other) && !(i2s->quirks & QUIRK_NO_DEFAULTGATE))
...
#if 0 // defined(CONFIG_CPU_S3C2416)
...
#define S3C2450_IISPSR_PSRAEN               (1<<15)
        ...
        int reg ;
        switch (div_id) {
        case S3C2416_DIV_MCLK:
                reg = readl(i2s->addr + I2SMOD) & ~S3C2450_IIS0MOD_BFS_MASK;
                writel(reg | div, i2s->addr + I2SMOD);
                break;
        case S3C2416_DIV_BCLK:
                reg = readl(i2s->addr + I2SMOD) & ~(S3C2450_IIS0MOD_FS_MASK);
                writel(reg | div, i2s->addr + I2SMOD);
                break;
        case S3C2416_DIV_PRESCALER:
                reg = readl(i2s->addr + I2SPSR);
                reg &= ~(S3C2450_IISPSR_PS_MASK|S3C2450_IISPSR_PSRAEN);
                writel(reg , i2s->addr + I2SPSR);
