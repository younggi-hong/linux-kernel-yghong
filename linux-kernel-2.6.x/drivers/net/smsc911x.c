			   ...
			   u16 val)
{
	...
	/* Confirm MII not busy */
	/*SHKO : SMSC_LAN9220db.pdf 파일의 5.4.7 참조. */
	if (unlikely(smsc911x_mac_read(pdata, MII_ACC) & MII_ACC_MII_BUSY_)) {
		SMSC_WARNING(HW,
...
static int smsc911x_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	struct phy_device *phy_dev = pdata->phy_dev;		/* SHKO, Added */
	unsigned long flags;		/* SHKO, Added */
	unsigned int 	reg_data;	/* SHKO, Added */
	unsigned int   *data_ptr;	/* SHKO, Added */
	unsigned short   *data_sptr;	/* SHKO, Added */
...
#if 0	//SHKO, Origin
	printk("[smsc911x_do_ioctl]cmd=0x%08x\n", cmd);
	return phy_mii_ioctl(pdata->phy_dev, ifr, cmd);
#else
	switch(cmd)
	{
		case SIOCGMIIPHY:
			phy_dev->addr = phy_dev->phy_id;
			return 0;

		case SIOCGMIIREG:
			//spin_lock_irqsave(&pdata->dev_lock, flags);
			printk("[smsc911x_do_ioctl]cmd=0x%08x, sizeof(struct ifreq)=%d\n", cmd, sizeof(struct ifreq));

			//copy_from_user((unsigned char *)&kernel_ifr, (unsigned char *)ifr, sizeof(struct ifreq));

			//data_sptr = (unsigned short   *)(kernel_ifr.ifr_data);
			data_sptr = (unsigned short   *)(ifr->ifr_data);
			//printk("[smsc911x_do_ioctl]cmd=0x%08x, kernel_ifr.ifr_data[0]=0x%08x, 0x%08x\n", cmd, data_sptr, *data_sptr);
			printk("[smsc911x_do_ioctl]cmd=0x%08x, ifr->ifr_data[0]=0x%08x, ifr->ifr_data[1]=0x%08x\n", cmd, data_sptr[0], data_sptr[1]);
			printk("[smsc911x_do_ioctl]phy_dev->addr=0x%08x\n", phy_dev->addr);
			reg_data = (unsigned int)smsc911x_mii_read(phy_dev->bus, phy_dev->addr, (int)data_sptr[1]);
			//reg_data = (unsigned int)smsc911x_mii_read(phy_dev->bus, phy_dev->addr, (int)0);
			printk("[smsc911x_do_ioctl]reg_data=0x%08x\n", reg_data);

			data_ptr = (unsigned int *)(&data_sptr[2]);

			*data_ptr = reg_data;
			printk("[smsc911x_do_ioctl]ifr.ifr_data[2]=0x%08x\n", data_ptr[0]);
			//copy_to_user((void *)arg, (const void *)&ctrl_info, 8);
			//spin_unlock_irqrestore(&pdata->dev_lock, flags);
	...
	printk("[smsc911x_do_ioctl]unsupported ioctl cmd");
	return -1;
...
static void __devinit smsc911x_read_mac_address(struct net_device *dev)
{
	...
	/* SHKO, Added */
	printk("[smsc911x_read_mac_address] eth_mac_addr=%02x:%02x:%02x:%02x:%02x:%02x\n", 
...
static int __devinit smsc911x_drv_probe(struct platform_device *pdev)
{
	...
	/* SHKO : res_size = 65536(==64KB), Mach-mango2450.c (arch\arm\mach-s3c2416)	파일의 mango24_smsc911x_resources 배열 참조 */
	...
	printk("[smsc911x_drv_probe] SHKO, res_size = %d\n", res_size);
	printk("[smsc911x_drv_probe] SHKO, res->start = 0x%08x\n", res->start);
	/* SHKO : res_size = 65536(==64KB), Mach-mango2450.c (arch\arm\mach-s3c2416)	파일의 mango24_smsc911x_resources 배열 참조 */
	/* SHKO : res->start = 0x28000000, Mach-mango2450.c (arch\arm\mach-s3c2416)	파일의 mango24_smsc911x_resources 배열 참조 */
	...
	/* SHKO, irq_res->start = 0x00000039 = 57 */
	/* SHKO : Mach-mango2450.c (arch\arm\mach-s3c2416) 파일의 mango24_smsc911x_resources 배열 참조 */
	dev->irq = irq_res->start;
	irq_flags = irq_res->flags & IRQF_TRIGGER_MASK;
	pdata->ioaddr = ioremap_nocache(res->start, res_size);

	/* SHKO, pdata->ioaddr = 0xc4880000 */
	printk("[smsc911x_drv_probe] SHKO, pdata->ioaddr = 0x%08x\n", pdata->ioaddr);
	...
	printk("[smsc911x_drv_probe] SHKO : dev->irq = %d\n", dev->irq);
	/* SHKO, dev->irq = 0x00000039 = 57 */
	/* SHKO : Mach-mango2450.c (arch\arm\mach-s3c2416) 파일의 mango24_smsc911x_resources 배열 참조 */
	...
	printk("[smsc911x_drv_probe] SHKO : after request_irq\n");
